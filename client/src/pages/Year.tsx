...
export default function Year() {
  ...
  const { data: year, isLoading: yearLoading } = trpc.studyPlan.yearsByProgramId.useQuery(
    { programId: 0 },
    { enabled: false }
  );

  const { data: semesters, isLoading: semestersLoading } = trpc.studyPlan.semestersByYearId.useQuery(
    { yearId: yearId! },
    { enabled: !!yearId }
  );
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-100">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h1 className="text-3xl font-bold text-slate-900">Academic Year</h1>
                <p className="text-sm text-slate-600">Select a semester to view courses</p>
              </div>
            </div>
          </div>
        </div>
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <h2 className="text-2xl font-bold text-slate-900 mb-8">Semesters</h2>
        ...
        {semesters && semesters.length > 0 ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {semesters.map((semester) => (
              <button
                key={semester.id}
                onClick={() => setLocation(`/semester/${semester.id}`)}
          ...
          <Card className="p-12 text-center border-0 shadow-lg bg-white">
            <p className="text-slate-600 mb-4">No semesters available for this year yet.</p>
          </Card>
        )}
      </section>
    </div>
